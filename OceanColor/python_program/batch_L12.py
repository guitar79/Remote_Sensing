...
def viirs_level12(ifile, root_name, prod_list, color_l2_file_fname, prod_list_sst, sst_l2_file_fname): 
    ...
    # Retrieve corresponding GEO file from OBPG archive...
    # ---
    fname_geo=  root_name[0:14] + '.GEO-M_SNPP.nc'
    url_file= 'http://oceandata.sci.gsfc.nasa.gov/cgi/getfile/' + fname_geo    
    cmd= 'wget ' + url_file           
...
def batch_proc_L12(l1a_dir, l2_dir='not_specified', prod_list='OC_suite', prod_list_sst='none', swir_onoff='off', hires='off'):
    ...
    if root_name[0][0] != 'L': root_name_trim = [name[0:14] for name in root_name] #list of file names without extension (non landsat)
    if root_name[0][0] == 'L': root_name_trim = root_name[:]                       #for landsat do not trim the extension...
   
    frs_chk = np.asarray(['FRS' in name for name in root_name] )
    
    if frs_chk.all() == True:     
        color_l2_file_fname = [l2_dir + '/' + name + '.L2_OC_FRS'  for name in root_name_trim] #list of level 2 OC files to be created     
    else:
